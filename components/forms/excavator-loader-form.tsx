...
export function ExcavatorLoaderForm() {
  ...
  const [items, setItems] = useState<Record<string, CheckStatus>>(
    Object.fromEntries(excavatorLoaderItems.map((item) => [item, null]))
  )
    ...
    <form onSubmit={handleSubmit} className="mx-auto max-w-3xl space-y-6 p-4 pb-12 lg:p-8 lg:pb-16">
      ...
      <Card>
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-xl text-foreground">Excavator Loader Pre-Shift Inspection Checklist</CardTitle>
          <CardDescription>
      ...
      <div className="flex items-center justify-between text-sm">
        <span className="text-muted-foreground">
          Progress: {checkedCount} / {excavatorLoaderItems.length} items checked
        </span>
        {allItemsChecked && (
      ...
      <div className="h-2 overflow-hidden rounded-full bg-muted">
        <div
          ...
          style={{ width: `${(checkedCount / excavatorLoaderItems.length) * 100}%` }}
        />
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {excavatorLoaderItems.map((item, idx) => (
            <ChecklistRadioGroup
              key={item}
              label={item}
              value={items[item]}
